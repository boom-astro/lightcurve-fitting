...
| **Parametric** | fluxes | PSO model selection → SVI posterior inference | best-fit model (Bazin, Villar, TDE, Arnett, Magnetar, …), posterior means & uncertainties |
| **Thermal** | magnitudes | Blackbody color fitting via PSO | log temperature at peak, cooling rate |

## Quick start

```rust
use lightcurve_fitting::{build_mag_bands, build_flux_bands, fit_nonparametric, fit_parametric, fit_thermal};
